...
(function(){
    ...
    ComicToll.prototype.css3 = function(){
        Snap("#comicBBox").node.className="animate";
        Snap("#comicBBox").node.style.cssText +="-webkit-transform:translate3d(0,"+(-parseInt((2640-1008)*document.documentElement.clientWidth/640))+"px,0)";
        setTimeout(this.onEnd,14000);
    }
...
var Wel = {
    ...
    page2:function(){
         ccSVG.select("#page2").use(ccSVG.select("#notice")).attr({id:"nnotice"}).node.style.cssText="opacity:0";
    ...
    page3:function(){
        ...
        for(var i = 1;i<=5;i++){
                ccSVG.select("#path path:nth-child("+i+")").animate({d:ccSVG.select("#doc path:nth-child("+i+")").attr("d"),fill:'#040000',strokeWidth:"0"},500)
        }
        setTimeout(function(){
            var timer = 0;
        ...
        function replaceNotice(){
            ccSVG.select("#nnotice").node.style.cssText="opacity:1; -webkit-transition:all 0.3s linear;";
            ccSVG.select("#saveNotice").attr({opacity:0},300)
        }
    }
}
...
function handleComplete(evt) {
    console.log(evt)
     var  v = new Vivus('cover', {duration: 190,type:"oneByOne",start:"manual", animTimingFunction: Vivus.EASE,file: 'svg/cover.svg',onReady: function (myVivus) {
                v.play();
                Wel.index();
            } 
        });
